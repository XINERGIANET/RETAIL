        ...
        <form method="GET" class="flex flex-col gap-3 lg:flex-row lg:items-center">
            ...
            <div class="flex flex-wrap gap-2">
                <x-ui.button size="md" variant="primary" type="submit"
                    class="h-11 px-5 shadow-sm hover:shadow-md transition-all duration-200 active:scale-95 text-gray-100 color_button">
                    <i class="ri-search-line"></i>
                    <span>Buscar</span>
                </x-ui.button>
        ...
        <div
            ...
            <table class="w-full min-w-[1000px] border-collapse relative">
                <thead>
                    <tr class="text-white">
                        <th
                            class="px-4 py-4 text-center sm:px-5 first:rounded-tl-xl font-semibold text-[11px] uppercase whitespace-nowrap">
                            F. inicio
                        </th>
                        <th class="px-4 py-4 text-center sm:px-5 font-semibold text-[11px] uppercase whitespace-nowrap">
                            F. fin
                        </th>
                        <th class="px-4 py-4 text-center sm:px-5 font-semibold text-[11px] uppercase whitespace-nowrap">
                            Nro. de apertura
                        ...
                        <th class="px-4 py-4 text-left sm:px-5 font-semibold text-[11px] uppercase whitespace-nowrap">
                            Detalle de turno
                        </th>
                        <th class="px-4 py-4 text-center sm:px-5 font-semibold text-[11px] uppercase whitespace-nowrap">
                            Situación
                        ...
                        <th
                            class="px-4 py-4 text-center sm:px-5 last:rounded-tr-xl font-semibold text-[11px] uppercase whitespace-nowrap">
                            Operaciones
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($relations as $relation)
                        @php
                            ...
                            $openingNumber =
                                $startMovement?->movement?->number ??
                                str_pad((string) $relation->id, 8, '0', STR_PAD_LEFT);
                            $showUrl =
                                $startMovement?->movement_id && $startMovement?->cash_register_id
                                    ? route(
                                        'admin.petty-cash.show',
                                        [
                                            'cash_register_id' => $startMovement->cash_register_id,
                                            'movement' => $startMovement->movement_id,
                                        ] + ($viewId ? ['view_id' => $viewId] : []),
                                    )
                                    : '#';
                        @endphp
                        <tr
                            class="border-b border-gray-100 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-white/5 transition">
                            ...
                            <td class="px-4 py-4 sm:px-5 text-xs text-gray-700 align-middle text-center">
                                ...
                                @else
                                    <x-ui.badge variant="light" color="success" class="text-[10px] px-2 py-0.5">En
                                        Curso</x-ui.badge>
                                @endif
                            </td>
                            <td
                                class="px-4 py-4 sm:px-5 text-xs font-bold text-gray-800 dark:text-white/90 align-middle text-center">
                                {{ $openingNumber }}
                            </td>
                                    ...
                                    @foreach ($relation->turn_summary ?? collect() as $entry)
                                        <div class="space-y-0.5">
                                            <p class="font-bold text-gray-800 dark:text-white/90 flex items-center gap-1.5">
                                                <i
                                                    class="{{ $entry['icon'] ?? 'ri-file-list-line' }} text-brand-600 text-sm"></i>
                                                {{ $entry['label'] }} (PEN
                                                {{ number_format((float) ($entry['total'] ?? 0), 1) }}):
                                            </p>
                                            @foreach ($entry['details'] ?? [] as $detail)
                                                <p class="ml-6 text-[10px] text-gray-500 flex items-center gap-2">
                                                    <span class="w-1 h-1 rounded-full bg-gray-200"></span>
                                                    {{ $detail['method'] }}:
                                                    {{ number_format((float) ($detail['amount'] ?? 0), 1) }}
                                                </p>
                                            @endforeach
                                        </div>
                                    @endforeach
                            ...
                            <td class="px-4 py-4 sm:px-5 text-xs text-center align-middle">
                                @if ((string) $relation->status === '1')
                                    <x-ui.badge variant="light" color="success"
                                        class="text-[10px] px-2 py-0.5">Activado</x-ui.badge>
                                @else
                                    <x-ui.badge variant="light" color="gray"
                                        class="text-[10px] px-2 py-0.5">Cerrado</x-ui.badge>
                                @endif
                            </td>
