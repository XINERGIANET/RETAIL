...
@section('content')
    <div x-data="{}">
        @php
            ...
            $resolveTextColor = fn($operation) => str_contains($operation->action ?? '', 'product-types.create') ||
            str_contains($operation->action ?? '', 'product_types.create')
                ? '#111827'
                : '#FFFFFF';
        @endphp
        ...
        <x-common.component-card title="Tipos de producto" desc="Configura los tipos de producto por sucursal.">
                ...
                <form method="GET" class="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <div class="relative flex-1">
                        ...
                        <input type="text" name="search" value="{{ $search }}"
                            placeholder="Buscar por nombre, descripcion o comportamiento"
                    ...
                    <div class="flex flex-wrap gap-2">
                        <x-ui.button size="md" variant="primary" type="submit"
                            class="flex-1 sm:flex-none h-11 px-6 color_button">
                            <i class="ri-search-line text-gray-100"></i>
                            <span class="font-medium text-gray-100">Buscar</span>
                        </x-ui.button>
                        <x-ui.link-button size="md" variant="outline"
                            href="{{ route('product-types.index', $viewId ? ['view_id' => $viewId] : []) }}"
                            class="flex-1 sm:flex-none h-11 px-6">
                ...
                <div class="flex items-center gap-2">
                    @foreach ($topOperations as $operation)
                        @php
                            ...
                            $isCreate =
                                str_contains($action, 'product-types.create') ||
                                str_contains($action, 'product_types.create') ||
                                str_contains($action, 'create');
                            $createStyle = 'background-color: #12f00e; color: #111827;';
                        @endphp
                        @if ($isCreate)
                            <x-ui.button size="md" variant="primary" type="button" style="{{ $createStyle }}"
                                @click="$dispatch('open-product-type-modal')">
                        ...
                        @else
                            ...
                            <x-ui.link-button size="md" variant="primary" style="{{ $topStyle }}"
                                href="{{ $resolveActionUrl($operation->action ?? '', null, $operation) }}">
                                <i class="{{ $operation->icon }}"></i>
                                <span>{{ $operation->name }}</span>
                            </x-ui.link-button>
                        @endif
            ...
            <div
                ...
                <table class="w-full min-w-[900px]">
                    <thead>
                        <tr class="text-white">
                            <th class="px-5 py-3 text-left sm:px-6 first:rounded-tl-xl w-1/3">
                                <p class="font-semibold text-white text-theme-xs uppercase">Nombre</p>
                            </th>
                            <th class="px-5 py-3 text-center sm:px-6">
                                <p class="font-semibold text-white text-theme-xs uppercase">Comportamiento</p>
                            </th>
                            <th class="px-5 py-3 text-center sm:px-6">
                            ...
                            <tr
                                ...
                                <td class="px-5 py-4 text-left sm:px-6 w-1/3">
                                    <div class="flex items-center justify-start gap-3">
                                        @if ($productType->icon)
                                            <span
                                                class="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-200">
                                ...
                                <td class="px-5 py-4 text-center sm:px-6">
                                    <x-ui.badge variant="light"
                                        color="{{ $productType->behavior === 'SUPPLY' ? 'warning' : 'success' }}">
                                            ...
                                            @foreach ($rowOperations as $operation)
                                                @php
                                                    ...
                                                    $isEdit =
                                                        ...
                                                        str_contains($action, 'product-types.edit') ||
                                                        str_contains($action, 'product_types.edit');
                                                    $updateUrl = route('product-types.update', $productType);
                                                    $actionUrl = $isEdit
                                                        ? $updateUrl
                                                        : $resolveActionUrl($action, $productType, $operation);
                                                ...
                                                @if ($isDelete)
                                                    <form method="POST" action="{{ $actionUrl }}"
                                                        ...
                                                        data-swal-text="Se eliminara {{ $productType->name }}. Esta accion no se puede deshacer."
                                                ...
                                                @else
                                                    <div class="relative group">
                                                        @if ($isEdit)
                                                            <x-ui.button size="icon" variant="edit" type="button"
                                                                className="rounded-xl" style="{{ $editStyle }}"
                                                                aria-label="{{ $operation->name }}"
                                                        ...
                                                        @else
                                                            <x-ui.link-button size="icon"
                                                                variant="{{ $variant }}" href="{{ $actionUrl }}"
                                        ...
                                        @else
                                            <div class="relative group">
                                                <x-ui.button size="icon" variant="edit" type="button"
                                                    ...
                                                    style="background-color: #FBBF24; color: #111827;" aria-label="Editar"
                                                ...
                                                <span
                                                    class="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-3 whitespace-nowrap rounded-md bg-gray-900 px-2.5 py-1 text-xs text-white opacity-0 transition group-hover:opacity-100 z-[100] shadow-xl">Editar</span>
                                            </div>
                                            <form method="POST"
                                                action="{{ route('product-types.destroy', array_merge([$productType], $viewId ? ['view_id' => $viewId] : [])) }}"
                                                class="relative group js-swal-delete" data-swal-title="Eliminar tipo?"
                                                data-swal-text="Se eliminara {{ $productType->name }}. Esta accion no se puede deshacer."
                                                data-swal-confirm="Si, eliminar" data-swal-cancel="Cancelar"
                                                data-swal-confirm-color="#ef4444" data-swal-cancel-color="#6b7280">
                                                @csrf
                                                ...
                                                <span
                                                    class="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-3 whitespace-nowrap rounded-md bg-gray-900 px-2.5 py-1 text-xs text-white opacity-0 transition group-hover:opacity-100 z-[100] shadow-xl">Eliminar</span>
                                            </form>
                                        @endif
                                    </div>
                                </td>
            ...
            <div class="mt-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="text-sm text-gray-500 dark:text-gray-400">
                    Mostrando
                    <span
                        class="font-semibold text-gray-700 dark:text-gray-200">{{ $productTypes->firstItem() ?? 0 }}</span>
                    -
                    <span
                        class="font-semibold text-gray-700 dark:text-gray-200">{{ $productTypes->lastItem() ?? 0 }}</span>
                    de
        ...
        <x-ui.modal x-data="{ open: false }" @open-product-type-modal.window="open = true"
            @close-product-type-modal.window="open = false" :isOpen="false" :showCloseButton="false" class="w-full max-w-3xl">
        ...
        <x-ui.modal x-data="{ open: false, editPayload: null }"
            @open-product-type-edit-modal.window="editPayload = $event.detail; open = true"
            @close-product-type-edit-modal.window="open = false" :isOpen="false" :showCloseButton="true"
                ...
                <template x-if="editPayload">
                    <form :action="editPayload.updateUrl" method="POST" class="space-y-6">
                        ...
                        <div class="grid gap-5 sm:grid-cols-2">
                            <div>
                                <label
                                    class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Nombre</label>
                                <input type="text" name="name" x-model="editPayload.productType.name" required
                                    placeholder="Ingrese el nombre"
                                    class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                            </div>
                            <div x-data="iconPicker()" x-init="init()"
                                :data-initial-icon="editPayload?.productType?.icon ?? ''" class="relative">
                                <label
                                    class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Icono</label>
                                <div class="relative">
                                    <span
                                    ...
                                    <input type="text" name="icon" x-ref="iconInput" x-model="search"
                                        placeholder="Busca o escribe un icono..." autocomplete="off" spellcheck="false"
                                    ...
                                    <button type="button"
                                        ...
                                        @click="toggleDropdown()" aria-label="Abrir selector de iconos">
                                        <i class="ri-arrow-down-s-line text-lg transition-transform"
                                ...
                                <div x-show="open"
                                    ...
                                    <div class="mb-2 flex items-center justify-between text-xs text-gray-500">
                                        <span
                                            x-text="loading ? 'Cargando iconos...' : `${filteredIcons.length} iconos`"></span>
                                        <button type="button" class="text-brand-500 hover:text-brand-600"
                                    ...
                                    <template x-if="!loading && error">
                                        <div
                                            class="rounded-lg border border-dashed border-gray-200 px-3 py-4 text-center text-sm text-gray-500 dark:border-gray-800">
                                    ...
                                    <div class="max-h-64 overflow-y-auto custom-scrollbar"
                                        x-show="!loading && !error && displayedIcons.length">
                                        ...
                                        <div class="mt-2 text-xs text-gray-400 text-center"
                                            x-show="filteredIcons.length > displayedIcons.length">
                            ...
                            <div>
                                <label
                                    class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Comportamiento</label>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea name="description" rows="3" x-model="editPayload.productType.description"
