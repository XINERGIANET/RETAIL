...
@section('content')
    ...
    <x-common.component-card title="Listado de pasarela de pago"
        desc="Gestiona las pasarela de pago registrados en el sistema.">
        <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <form method="GET" class="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <div class="flex-none">
                    <x-form.select-autocomplete name="per_page" :value="$perPage ?? 10" :options="collect($allowedPerPage ?? [10, 20, 50, 100])
                        ->map(fn($n) => ['value' => $n, 'label' => $n . ' / página'])
                        ->values()
                        ->all()" placeholder="Por página"
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <x-ui.link-button size="md" variant="outline"
                        href="{{ $viewId ? route('admin.payment_gateways.index', ['view_id' => $viewId]) : route('admin.payment_gateways.index') }}"
            ...
            <div class="flex flex-wrap items-center gap-2">
                @foreach ($topOperations as $operation)
                    @php
                        ...
                        $isCreate =
                            str_contains($operation->action ?? '', 'payment_gateways.create') ||
                            ...
                            <tr
                                ...
                                <td class="px-5 py-4 sm:px-6 text-center">
                                    <x-ui.badge variant="light"
                                        color="{{ $paymentGateway->status ? 'success' : 'error' }}">
        ...
        <div class="mt-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div class="text-sm text-gray-500 dark:text-gray-400">
                Mostrando
                <span
                    class="font-semibold text-gray-700 dark:text-gray-200">{{ $paymentGateways->firstItem() ?? 0 }}</span>
                -
                <span
                    class="font-semibold text-gray-700 dark:text-gray-200">{{ $paymentGateways->lastItem() ?? 0 }}</span>
                de
    ...
    <x-ui.modal x-data="{ open: false }" @open-create-payment-gateway-modal.window="open = true"
        ...
        <div class="p-6 space-y-4">
            ...
            @endif
            <form id="create-payment-gateway-form" class="space-y-4"
                action="{{ $viewId ? route('admin.payment_gateways.store') . '?view_id=' . $viewId : route('admin.payment_gateways.store') }}"
                method="POST">
                @csrf
                @if ($viewId)
                    <input type="hidden" name="view_id" value="{{ $viewId }}">
