    ...
    ">
        ...
        <x-common.component-card title="GestiÃ³n de Movimientos" desc="Control de ingresos, egresos y traslados de fondos.">
            ...
            <div class="flex flex-col gap-5">
                ...
                <form method="GET" action="{{ route('admin.petty-cash.index', ['cash_register_id' => $selectedBoxId]) }}"
                    ...
                    @php
                        ...
                        foreach ($shiftRelations ?? [] as $rel) {
                            ...
                            $started = $rel->started_at
                                ? \Carbon\Carbon::parse($rel->started_at)->format('Y-m-d H:i:s')
                                : '';
                        ...
                        $conceptOptions = collect([['value' => '', 'label' => 'Todos']])
                            ->merge(
                                collect($paymentConceptsForFilter ?? [])->map(
                                    fn($c) => ['value' => (string) $c->id, 'label' => $c->description],
                                ),
                            )
                            ->values()
                            ->all();
                    ...
                    <div class="flex flex-wrap items-end gap-3 xl:flex-nowrap">
                        <div class="w-full sm:w-[140px] sm:flex-shrink-0">
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Por
                                página</label>
                            <x-form.select-autocomplete name="per_page" :value="$perPage ?? 10" :options="collect([10, 20, 50, 100])
                        ...
                        <div class="w-full sm:w-[120px] sm:flex-shrink-0" x-data="{
                            ...
                            <x-form.select-autocomplete name="cash_register_id" :value="$selectedBoxId" :options="collect($cashRegisters ?? [])
                                ->map(fn($r) => ['value' => $r->id, 'label' => $r->number])
                                ->values()
                                ->all()"
                                placeholder="Caja" :submit-on-change="false"
                        ...
                        <div class="flex-1 min-w-0 xl:min-w-[180px]">
                            ...
                            <x-form.select-autocomplete name="shift_relation_id" :value="$selectedShiftId" :options="$pettyShiftOptions->values()->all()"
                    ...
                    <div class="flex flex-wrap items-end gap-3 xl:flex-nowrap">
                        <div class="w-full sm:w-[140px] sm:flex-shrink-0">
                            ...
                            <x-form.select-autocomplete name="tipo_movimiento" :value="$selectedTipoMovimiento ?? 'all'" :options="[
                        ...
                        <div class="w-full sm:w-[200px] sm:flex-shrink-0">
                            <label
                                class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Concepto</label>
                            <x-form.select-autocomplete name="payment_concept_id" :value="$selectedPaymentConceptId ?? ''" :options="$conceptOptions"
                                placeholder="Todos" :submit-on-change="true"
                                inputClass="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
                        </div>
                        <div class="flex-1 min-w-0 xl:min-w-[150px]">
                            <x-form.date-picker id="petty-date-from" name="date_from" label="Desde" :defaultDate="$dateFrom ?? null"
                                dateFormat="Y-m-d" placeholder="dd/mm/yyyy" />
                        </div>
                        <div class="flex-1 min-w-0 xl:min-w-[150px]">
                            <x-form.date-picker id="petty-date-to" name="date_to" label="Hasta" :defaultDate="$dateTo ?? null"
                                dateFormat="Y-m-d" placeholder="dd/mm/yyyy" />
                        </div>
                        <div class="flex flex-shrink-0 items-end gap-2">
                            <x-ui.button type="submit" size="md" variant="primary" class="h-11 px-5 color_button    ">
                    ...
                    @if ($topOperations->isNotEmpty())
                        ...
                        @foreach ($topOperations as $operation)
                            @php
                                ...
                                $isIncomeOp =
                                    str_contains($topActionLower, 'ingreso') || str_contains($topNameLower, 'ingreso');
                                $isExpenseOp =
                                    str_contains($topActionLower, 'egreso') || str_contains($topNameLower, 'egreso');
                                $isOpenOp =
                                    str_contains($topActionLower, 'apertura') ||
                                    str_contains($topNameLower, 'apertura');
                                $isCloseOp =
                                    str_contains($topActionLower, 'cierre') ||
                                ...
                                $targetViewId = !empty($operation->view_id_action)
                                    ? $operation->view_id_action
                                    : $viewId;
            ...
            <div class="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
                <div class="flex items-center gap-2 text-sm text-gray-500">
                    ...
                    <x-ui.badge size="sm" variant="light" color="success">S/
                        {{ number_format((float) ($cashEfectivoTotal ?? 0), 2) }}</x-ui.badge>
                </div>
            </div>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="text-white">
                            ...
                            <th class="hidden px-3 py-3 text-left">
                    ...
                    @forelse ($movements as $movement)
                        @php
                            ...
                            $paymentSummary = collect($movement->cashMovement?->details ?? [])
                                ...
                                ->map(
                                    fn($items, $method) => trim(
                                        ($method ?: 'Metodo') . ': ' . number_format($items->sum('amount'), 2),
                                    ),
                                )
                                ->values()
                                ->implode(' | ');
                        ...
                        <tbody>
                            <tr
                                ...
                                <td class="px-4 py-4 text-center">
                                    <button type="button"
                                        @click="openRow === {{ $movement->id }} ? openRow = null : openRow = {{ $movement->id }}"
                                ...
                                <td class="px-3 py-3 whitespace-nowrap align-middle">
                                    <x-ui.badge variant="light"
                                        color="{{ $isIngreso ? 'success' : 'error' }}">{{ $isIngreso ? 'Ingreso' : 'Egreso' }}</x-ui.badge>
                                </td>
                                <td class="px-3 py-3 align-middle">
                                    <x-ui.badge variant="light" color="warning"
                                        class="text-[10px]">{{ $conceptName }}</x-ui.badge>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap align-middle">
                                    <p class="font-bold text-gray-800 text-theme-sm dark:text-white/90">S/
                                        {{ number_format($movement->cashMovement?->total ?? 0, 2) }}</p>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap align-middle">
                                    <div>
                                        <p class="text-gray-800 text-[11px] font-medium dark:text-white/90">
                                            {{ $movement->moved_at ? $movement->moved_at->format('j/m/Y') : '-' }}</p>
                                        <p class="text-gray-500 text-[10px] dark:text-gray-400">
                                            {{ $movement->moved_at ? $movement->moved_at->format('h:i:s A') : '' }}</p>
                                    </div>
                                </td>
                                <td class="px-3 py-3 align-middle">
                                    <p class="max-w-[280px] truncate text-[11px] font-medium text-gray-700 dark:text-gray-200"
                                        title="{{ $paymentSummary ?: '-' }}">{{ $paymentSummary ?: '-' }}</p>
                                </td>
                                <td class="hidden px-3 py-3 align-middle">
                                    <p class="text-gray-800 text-[11px] font-bold dark:text-white/90">
                                        {{ $movement->user_name ?: '-' }}</p>
                                </td>
                                <td class="hidden px-3 py-3 align-middle">
                                    <p class="text-gray-500 text-[10px] dark:text-gray-400 capitalize">
                                ...
                                <td class="px-3 py-3 align-middle text-center">
                                    <x-ui.badge variant="light"
                                        color="{{ $isActive ? 'success' : 'error' }}">{{ $isActive ? 'Activado' : 'Desactivado' }}</x-ui.badge>
                                </td>
                                <td class="px-3 py-3">
                            ...
                            <tr x-show="openRow === {{ $movement->id }}" x-cloak
                                class="border-b border-gray-100 bg-gray-50/70 dark:bg-gray-800/40 dark:border-gray-800 transition-all duration-300">
                                    ...
                                    <div class="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6 mb-2 max-w-6xl">
                                        <div
                                            ...
                                            <p class="mt-0.5 truncate text-sm font-semibold text-gray-700 dark:text-gray-200"
                                                title="{{ $movement->person_name ?: '-' }}">
                                                {{ $movement->person_name ?: '-' }}</p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50">
                                            <p
                                                ...
                                                Responsable</p>
                                            <p class="mt-0.5 truncate text-sm font-semibold text-gray-700 dark:text-gray-200"
                                                title="{{ $movement->responsible_name ?: '-' }}">
                                                {{ $movement->responsible_name ?: '-' }}</p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50">
                                            <p
                                        ...
                                        <div
                                            ...
                                            <p class="mt-0.5 text-sm font-semibold text-gray-700 dark:text-gray-200">
                                                {{ $movement->cashMovement?->currency ?: 'PEN' }}</p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50 lg:col-span-2">
                                            <p
                                                class="text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">
                                                Origen</p>
                                            <p
                                                class="mt-0.5 truncate text-sm font-semibold text-gray-700 dark:text-gray-200">
                                                @php
                                                ...
                                                @endphp
                                                {{ $originType }} -
                                                {{ $originDocPrefix }}{{ $originSeries ? $originSeries . '-' : '' }}{{ $movement->number }}
                                            </p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50">
                                        ...
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50 lg:col-span-6">
                                            <p
                                                class="text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">
                                                Comentario</p>
                                            <p class="mt-0.5 text-sm font-semibold text-gray-700 dark:text-gray-200">
                ...
                <div class="mb-4 flex items-center justify-between">
                    <div>
                        <p class="text-[10px] uppercase tracking-[0.2em] text-gray-400">
                            <span
                                x-text="formDocId == ingresoId ? 'OperaciÃ³n de Ingreso' : 'OperaciÃ³n de Egreso'"></span>
                        </p>
