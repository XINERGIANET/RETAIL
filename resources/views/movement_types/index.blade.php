...
@section('content')
    <div x-data="{}">
        ...
        <x-common.component-card title="Tipos de movimiento" desc="Gestiona los tipos de movimiento.">
                    ...
                    @foreach ($topOperations as $operation)
                        ...
                        @if ($isCreate)
                            <x-ui.button size="md" variant="primary" type="button" style="{{ $topStyle }}"
                                @click="$dispatch('open-movement-type-modal')">
            ...
            <div class="mt-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="text-sm text-gray-500 dark:text-gray-400">
                    Mostrando
                    <span
                        class="font-semibold text-gray-700 dark:text-gray-200">{{ $movementTypes->firstItem() ?? 0 }}</span>
                    -
                    <span
                        class="font-semibold text-gray-700 dark:text-gray-200">{{ $movementTypes->lastItem() ?? 0 }}</span>
                    de
        ...
        <x-ui.modal x-data="{ open: false }" @open-movement-type-modal.window="open = true"
            @close-movement-type-modal.window="open = false" :isOpen="false" :showCloseButton="false" class="max-w-3xl">
            <div class="p-6 sm:p-8">
                ...
                <form method="POST"
                    action="{{ $viewId ? route('admin.movement-types.store') . '?view_id=' . $viewId : route('admin.movement-types.store') }}"
                    class="space-y-6">
                    @csrf
                    @if ($viewId)
                        <input type="hidden" name="view_id" value="{{ $viewId }}">
