...
@section('content')
    ...
    <x-common.component-card title="Listado de métodos de pago"
        desc="Gestiona los métodos de pago registrados en el sistema.">
            ...
            <form method="GET" class="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <x-ui.link-button size="md" variant="outline"
                        href="{{ $viewId ? route('admin.payment_methods.index', ['view_id' => $viewId]) : route('admin.payment_methods.index') }}"
                                            ...
                                            @if ($isDelete)
                                                @php
                                                    ...
                                                    if ($viewId && $destroyFormUrl !== '#') {
                                                        ...
                                                        $destroyFormUrl .=
                                                            $destroySep . 'view_id=' . urlencode($viewId);
                                                    }
                                                @endphp
                                                <form action="{{ $destroyFormUrl }}" method="POST"
                                                    ...
                                                    <x-ui.button size="icon" variant="{{ $variant }}"
                                                        type="submit" className="rounded-xl" style="{{ $buttonStyle }}"
        ...
        <div class="mt-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div class="text-sm text-gray-500 dark:text-gray-400">
                Mostrando
                <span
                    class="font-semibold text-gray-700 dark:text-gray-200">{{ $paymentMethods->firstItem() ?? 0 }}</span>
                -
    ...
    <x-ui.modal x-data="{ open: false }" @open-create-payment-method-modal.window="open = true"
        ...
        <div class="p-6 space-y-4">
            ...
            <form id="create-payment-method-form" class="space-y-4"
                action="{{ $viewId ? route('admin.payment_methods.store') . '?view_id=' . $viewId : route('admin.payment_methods.store') }}"
                method="POST">
                @csrf
                @if ($viewId)
                    <input type="hidden" name="view_id" value="{{ $viewId }}">
