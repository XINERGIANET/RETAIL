...
@section('content')
    <div x-data="{}">
        ...
        <x-common.component-card title="Tipos de documento" desc="Gestiona los tipos de documento.">
                    ...
                    @foreach ($topOperations as $operation)
                        ...
                        @if ($isCreate)
                            <x-ui.button size="md" variant="primary" type="button" style="{{ $topStyle }}"
                                @click="$dispatch('open-document-type-modal')">
                            ...
                            <tr
                                ...
                                <td class="px-5 py-4 sm:px-6 sticky-left text-center">
                                    <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90 truncate"
                                        title="{{ $documentType->name }}">{{ $documentType->name }}</p>
                                </td>
                                ...
                                <td class="px-5 py-4 sm:px-6 text-center">
                                    <p class="text-gray-500 text-theme-sm dark:text-gray-400">
                                        {{ $documentType->movementType?->description ?? '-' }}</p>
                                </td>
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-12">
                                    <div class="flex flex-col items-center gap-3 text-center text-sm text-gray-500">
                                        ...
                                        <p class="text-base font-semibold text-gray-700 dark:text-gray-200">No hay tipos
                                            registrados.</p>
            ...
            <div class="mt-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="text-sm text-gray-500 dark:text-gray-400">
                    Mostrando
                    <span
                        class="font-semibold text-gray-700 dark:text-gray-200">{{ $documentTypes->firstItem() ?? 0 }}</span>
                    -
                    <span
                        class="font-semibold text-gray-700 dark:text-gray-200">{{ $documentTypes->lastItem() ?? 0 }}</span>
                    de
        ...
        <x-ui.modal x-data="{ open: false }" @open-document-type-modal.window="open = true"
            @close-document-type-modal.window="open = false" :isOpen="false" :showCloseButton="false" class="max-w-4xl">
            <div class="p-6 sm:p-8">
                ...
                <form method="POST"
                    action="{{ $viewId ? route('admin.document-types.store') . '?view_id=' . $viewId : route('admin.document-types.store') }}"
                    class="space-y-6">
                    @csrf
                    @if ($viewId)
                        <input type="hidden" name="view_id" value="{{ $viewId }}">
