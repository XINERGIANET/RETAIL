...
@section('content')
    <div x-data="{}">
        <x-common.page-breadcrumb pageTitle="Permisos" :crumbs="[
            ['label' => 'Empresas', 'url' => route('admin.companies.index')],
            [
                'label' => $company->legal_name . ' | Sucursales',
                'url' => route('admin.companies.branches.index', $company),
            ],
            [
                'label' => $branch->legal_name . ' | Perfiles',
                'url' => route('admin.companies.branches.profiles.index', [$company, $branch]),
            ],
            ['label' => $profile->name . ' | Permisos'],
        ]" />

        <x-common.component-card title="Permisos de {{ $profile->name }}"
            ...
            <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                <form method="GET" class="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <div class="relative flex-1">
                        ...
                        <input type="text" name="search" value="{{ $search }}" placeholder="Buscar por permiso"
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <x-ui.link-button size="sm" variant="outline"
                            href="{{ route('admin.companies.branches.profiles.permissions.index', [$company, $branch, $profile]) }}">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <x-ui.button size="md" variant="primary" type="button"
                        style=" background-color: #12f00e; color: #111827;" @click="$dispatch('open-assign-permissions')">
                    ...
                    <tbody>
                        @forelse ($permissions as $permission)
                            <tr
                                ...
                                <td class="px-5 py-4 sm:px-6 text-center">
                                    <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $permission->module_name }}
                                    </p>
                                </td>
                                <td class="px-5 py-4 sm:px-6 text-center">
                                ...
                                <td class="px-5 py-4 sm:px-6 text-center">
                                    <div class="flex items-center justify-center">
                                        <form method="POST"
                                            ...
                                            @csrf
                                            @method('PATCH')
                                            <x-ui.button size="icon" variant="primary" type="submit"
                                                ...
                                                <i
                                                    class="{{ $permission->status ? 'ri-eye-line' : 'ri-eye-off-line' }}"></i>
                                            </x-ui.button>
                                            <span
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-12">
                                    <div class="flex flex-col items-center gap-3 text-center text-sm text-gray-500">
                                        ...
                                        <p class="text-base font-semibold text-gray-700 dark:text-gray-200">No hay permisos
                                            registrados.</p>
            ...
            <div class="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div class="text-sm text-gray-500">
                    Mostrando
                    <span
                        class="font-semibold text-gray-700 dark:text-gray-200">{{ $permissions->firstItem() ?? 0 }}</span>
                    -
        ...
        <x-ui.modal x-data="{ open: false }" @open-assign-permissions.window="open = true"
            @close-assign-permissions.window="open = false" :isOpen="false" :showCloseButton="false" class="max-w-6xl">
            <div class="p-6 sm:p-8">
                ...
                <form method="POST"
                    ...
                    class="space-y-6" data-select-scope>
                    @csrf
                    ...
                    @if ($modules->isEmpty())
                        <div
                            class="rounded-xl border border-dashed border-gray-200 px-4 py-6 text-center text-sm text-gray-500 dark:border-gray-800">
                    ...
                    @else
                        <div class="space-y-5">
                            @foreach ($modules as $module)
                                <div
                                    class="rounded-2xl border border-gray-200 bg-white/60 p-4 dark:border-gray-800 dark:bg-gray-900/40">
                                    <div class="mb-4 flex items-center gap-3">
                                        ...
                                        <div>
                                            <h4 class="text-base font-semibold text-gray-800 dark:text-white/90">
                                                {{ $module->name }}</h4>
                                            <p class="text-xs text-gray-500">Opciones: {{ $module->menuOptions->count() }}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                        @foreach ($module->menuOptions as $option)
                                            <label
                                                class="flex items-start gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 shadow-sm transition hover:border-brand-300 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-200">
                                                <input type="checkbox" name="permissions[]" value="{{ $option->id }}"
                                                    data-select-item
