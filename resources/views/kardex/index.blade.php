        ...
        <x-common.component-card title="Kardex" :desc="'Consulta el historial de movimientos de inventario por producto.' .
            ...
            <form method="GET" action="{{ route('kardex.index') }}" class="mb-6 space-y-5">
                ...
                <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    ...
                    <div>
                        <x-form.select-autocomplete name="product_id" :value="$productId" :options="collect($products)
                            ->map(fn($p) => ['value' => $p->id, 'label' => $p->code . ' - ' . $p->description])
                            ->prepend(['value' => 'all', 'label' => 'Todos los productos'])
                            ->values()
                            ->all()"
                            placeholder="Todos los productos" label="Producto"
                    ...
                    <div>
                        <x-form.select-autocomplete name="category_id" :value="$categoryId" :options="collect($categories)
                            ->map(fn($c) => ['value' => $c->id, 'label' => $c->description])
                            ->prepend(['value' => 'all', 'label' => 'Todas'])
                            ->values()
                            ->all()"
                            placeholder="Todas" label="Categoría"
                            inputClass="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
                    </div>
                    <div>
                        <x-form.select-autocomplete name="product_type_id" :value="$productTypeId" :options="collect($productTypes)
                            ->map(fn($t) => ['value' => $t->id, 'label' => $t->name])
                            ->prepend(['value' => 'all', 'label' => 'Todos'])
                            ->values()
                            ->all()"
                            placeholder="Todos" label="Tipo"
                            inputClass="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
                    </div>
                </div>
                ...
                <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    <div>
                        <x-form.date-picker id="kardex-date-from" name="date_from" label="Fecha inicio" :defaultDate="$dateFrom"
                            dateFormat="Y-m-d H:i" :enableTime="true" :time24hr="true" :altInput="true"
                            altFormat="d/m/Y H:i" locale="es" placeholder="dd/mm/yyyy hh:mm" />
                    </div>
                    <div>
                        <x-form.date-picker id="kardex-date-to" name="date_to" label="Fecha fin" :defaultDate="$dateTo"
                            dateFormat="Y-m-d H:i" :enableTime="true" :time24hr="true" :altInput="true"
                            altFormat="d/m/Y H:i" locale="es" placeholder="dd/mm/yyyy hh:mm" />
                    </div>
                    <div>
                        <x-form.select-autocomplete name="situation" :value="$situation" :options="[
                            ['value' => 'all', 'label' => 'Todos'],
                            ['value' => 'E', 'label' => 'Activado'],
                            ['value' => 'I', 'label' => 'Inactivo'],
                            ['value' => 'A', 'label' => 'Anulado'],
                        ]"
                            placeholder="Todos" label="Situación"
                ...
                <div class="flex flex-wrap items-end justify-start gap-2">
                    <x-ui.button type="submit" size="md" variant="primary" class="h-11 px-6 color_button">
                        <i class="ri-search-line"></i>
                    </x-ui.button>
                    <button type="submit" formaction="{{ route('kardex.pdf') }}" formtarget="_blank"
                    ...
                    <button type="button" id="kardex-excel-button"
                        class="inline-flex h-11 items-center gap-2 rounded-lg bg-green-500 px-6 text-sm font-semibold text-white shadow-theme-xs hover:bg-green-600">
                ...
                <table id="kardex-table" class="w-full" style="table-layout: fixed; min-width: 960px;">
                    <thead>
                        <tr class="text-white text-center" style="background-color: #334155;">
                            ...
                            <th class="px-2 py-2 text-xs font-semibold uppercase text-left" style="width: 120px;">Producto
                            </th>
                    ...
                    <tbody>
                        @forelse ($movements as $idx => $m)
                            ...
                            <tr
                                ...
                                <td class="px-1.5 py-2 text-center align-middle">
                                    <button type="button"
                                        ...
                                        <i class="ri-subtract-line text-sm" x-show="openRow === {{ (int) $rowId }}"
                                            x-cloak></i>
                                    </button>
                                </td>
                                <td class="px-2 py-2 text-sm text-left text-gray-800 dark:text-white/90 align-top overflow-hidden"
                                    style="width: 120px; max-width: 120px;">
                                    <div class="min-w-0 leading-tight break-words" style="max-width: 116px;">
                                        <div class="font-medium truncate"
                                            title="{{ $productLine1 }}{{ $productLine2 !== '' ? ' ' . $productLine2 : '' }}">
                                            {{ $productLine1 }}</div>
                                        @if ($productLine2 !== '')
                                            <div class="text-gray-600 dark:text-gray-400 mt-0.5 truncate"
                                                title="{{ $productLine2 }}">{{ $productLine2 }}</div>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="px-1.5 py-2 text-sm text-center text-gray-800 dark:text-white/90">
                                    @if ($dateValue)
                                        <div class="leading-tight text-xs">
                                            ...
                                            <div class="text-gray-600 dark:text-gray-400">{{ $dateValue->format('H:i') }}
                                            </div>
                                        </div>
                                    @else
                                        -
                                ...
                                <td
                                    class="px-2 py-2 text-sm text-center text-gray-600 dark:text-gray-400 text-xs overflow-hidden">
                                    <span class="truncate block"
                                        title="{{ $m['origin'] ?? '-' }}">{{ $m['origin'] ?? '-' }}</span>
                                </td>
                                <td class="px-1.5 py-2 text-center overflow-hidden">
                                    <span
                                        class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium text-white truncate max-w-full"
                                        style="{{ $situationStyle }}" title="{{ $situationLabel }}">
                                        {{ $situationLabel }}
                                    </span>
                                </td>
                                <td class="px-1.5 py-2 text-center">
                                    @if ($operationUrl && $operationLabel)
                                        <a href="{{ $operationUrl }}"
                                            class="inline-flex h-9 w-9 items-center justify-center rounded-full text-white transition hover:opacity-90"
                                            style="background-color: #E91E63;" title="{{ $operationLabel }}"
                            ...
                            <tr x-show="openRow === {{ (int) $rowId }}" x-cloak
                                    ...
                                    <div class="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6 w-full">
                                        ...
                                        <div
                                            ...
                                            <p
                                                ...
                                                Total</p>
                                            <p class="mt-0.5 text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ isset($m['total']) && $m['total'] != 0 ? 'S/ ' . number_format($m['total'], 2) : '-' }}
                                            </p>
                                        </div>
                                        @if ($operationUrl && $operationLabel)
                                            <div
                                                ...
                                                <p class="mt-0.5">
                                                    <a href="{{ $operationUrl }}"
                                                        class="inline-flex items-center gap-1.5 text-sm font-medium text-brand-600 hover:text-brand-700 dark:text-brand-400">
        ...
        (function() {
            ...
            if (excelButton) {
                excelButton.addEventListener('click', function() {
                    const headers = ['Producto', 'Categoria', 'Tipo', 'Unidad', 'Stock anterior', 'Cantidad',
                        'Stock actual', 'P. unitario', 'Moneda', 'Fecha', 'Origen', 'Situacion'
                    ];
                    ...
                    rows.forEach((row) => {
                        const values = [
                            ...
                            row.date || '',
                            row.origin || '-',
                            row.situation === 'A' ? 'Anulado' : (row.situation === 'I' ?
                                'Inactivo' : 'Activado'),
