@php
    ...
    $existingItems = collect($purchaseMovement?->details ?? [])
        ->map(function ($detail) {
            return [
...
<div x-data="purchaseForm({
...
})" x-init="initProvider()" class="space-y-5">
    <div class="space-y-5">
        <div class="rounded-2xl border border-gray-200 bg-white p-4">
            <h3 class="mb-4 text-sm font-bold uppercase tracking-wide text-gray-700">Cabecera de compra</h3>
            <div class="grid grid-cols-1 gap-3 md:grid-cols-12">
                <div class="md:col-span-2">
                    <x-form.date-picker name="moved_at" label="Fecha de compra" placeholder="dd/mm/yyyy hh:mm"
                        :defaultDate="old('moved_at', optional($purchase?->moved_at ?? now())->format('Y-m-d H:i'))" dateFormat="Y-m-d H:i" :enableTime="true" :time24hr="true" :altInput="true"
                ...
                <div class="md:col-span-4">
                    ...
                    <div class="relative" @click.outside="providerOpen = false">
                        <input type="text" x-model="providerQuery" @focus="providerOpen = true"
                            @input="providerOpen = true" @keydown.arrow-down.prevent="moveProviderCursor(1)"
                            ...
                            placeholder="Buscar proveedor por nombre o documento" autocomplete="off">
                        <button type="button" x-show="selectedProviderId" @click="clearProvider()"
                            class="absolute right-2 top-1/2 -translate-y-1/2 rounded p-1 text-gray-400 hover:text-gray-700"
                            title="Limpiar proveedor">
                            <i class="ri-close-line"></i>
                        </button>

                        <div x-show="providerOpen" x-cloak
                            ...
                            <template x-for="(provider, pIndex) in filteredProviders" :key="provider.id">
                                <button type="button" @click="selectProvider(provider)"
                                    ...
                                    <span class="font-medium text-gray-800"
                                        x-text="provider.label || 'SIN NOMBRE'"></span>
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="document_type_id" class="h-10 w-full rounded-xl border border-gray-300 px-3 text-sm"
                        required>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4">
            <div class="mb-4 flex items-center justify-between">
                ...
                <button type="button" @click="addItem()"
                    class="inline-flex items-center rounded-xl bg-[#244BB3] px-3.5 py-2 text-xs font-semibold text-white hover:bg-[#1f3f98]">
                        ...
                        <template x-for="(item, idx) in items" :key="idx">
                            <tr class="border-b border-gray-100 last:border-b-0">
                                <td class="relative overflow-visible px-2 py-1.5">
                                    ...
                                    <div class="relative z-20" @click.outside="item.product_open = false">
                                        <input type="text" x-model="item.product_query"
                                            @focus="item.product_open = true" @input="item.product_open = true"
                                            ...
                                            placeholder="Selecciona producto" autocomplete="off">
                                        <button type="button" x-show="item.product_id" @click="clearProduct(idx)"
                                            class="absolute right-2 top-1/2 -translate-y-1/2 rounded p-1 text-gray-400 hover:text-gray-700"
                                            title="Limpiar producto">
                                            <i class="ri-close-line"></i>
                                        </button>
                                        <div x-show="item.product_open" x-cloak
                                            ...
                                            <template x-for="(product, pIndex) in filteredProducts(item)"
                                                :key="product.id">
                                                <button type="button" @click="selectProduct(idx, product)"
                                                    @mouseenter="item.product_cursor = pIndex"
                                                    class="flex w-full items-center justify-between px-3 py-2 text-left text-sm hover:bg-gray-50"
                                                    :class="item.product_cursor === pIndex ? 'bg-gray-100' : ''">
                                                    <span class="font-medium text-gray-800"
                                                        x-text="`${product.code || 'SIN'} - ${product.name}`"></span>
                                                    <span class="text-xs text-gray-500"
                                                        x-text="product.unit_name || ''"></span>
                                                </button>
                                            </template>
                                        </div>
                                    </div>
                                ...
                                <td class="px-2 py-1.5">
                                    <select :name="`items[${idx}][unit_id]`" x-model.number="item.unit_id"
                                ...
                                <td class="px-2 py-1.5">
                                    <input :name="`items[${idx}][quantity]`" type="number" step="1"
                                        min="1" x-model.number="item.quantity"
                                        class="h-11 w-20 max-w-full rounded-lg border border-gray-300 px-2 text-center text-lg font-semibold"
                                        required>
                                </td>
                                <td class="px-2 py-1.5">
                                    <input :name="`items[${idx}][amount]`" type="number" step="0.01"
                                        min="0" x-model.number="item.amount"
                                        class="h-11 w-20 max-w-full rounded-lg border border-gray-300 px-2 text-center text-lg font-semibold"
                                        required>
                                </td>
                                <td class="px-2 py-1.5">
                                    <input :name="`items[${idx}][comment]`" x-model="item.comment"
                                        class="h-11 w-full rounded-lg border border-gray-300 px-3 text-sm"
                                        placeholder="Obs">
                                </td>
                                <td class="px-2 py-1.5 text-right">
                                    <p class="text-sm font-bold text-gray-800"
                                        x-text="money((item.quantity || 0) * (item.amount || 0))"></p>
                                </td>
                                <td class="px-2 py-1.5 text-center">
                                    <button type="button" @click="removeItem(idx)"
                                        ...
                                        title="Eliminar item">
                                        <i class="ri-delete-bin-line text-white"
                                            style="color:#fff !important; font-size:16px;"></i>
    ...
    <div class="space-y-5">
        <div class="rounded-2xl border border-gray-200 bg-white p-4">
            <div
                class="mb-4 grid grid-cols-2 overflow-hidden rounded-xl border border-[#fecaca] text-xs font-semibold uppercase">
            ...
            <div class="grid grid-cols-1 gap-3 md:grid-cols-6">
                <div class="md:col-span-2">
                    ...
                    <select name="detail_type" class="h-10 w-full rounded-xl border border-gray-300 px-3 text-sm">
                        @foreach (['DETALLADO', 'GLOSA'] as $option)
                            <option value="{{ $option }}" @selected(old('detail_type', $purchaseMovement->detail_type ?? 'DETALLADO') === $option)>{{ $option }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0" max="100" name="tax_rate_percent"
                        x-model.number="taxRate" class="h-10 w-full rounded-xl border border-gray-300 px-3 text-sm"
                        required>
                </div>
                <div>
                    ...
                    <select name="payment_type" class="h-10 w-full rounded-xl border border-gray-300 px-3 text-sm">
                        @foreach (['CONTADO', 'CREDITO'] as $option)
                            <option value="{{ $option }}" @selected(old('payment_type', $purchaseMovement->payment_type ?? 'CONTADO') === $option)>{{ $option }}
                            </option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="currency" class="h-10 w-full rounded-xl border border-gray-300 px-3 text-sm"
                        required>
                ...
                <div>
                    ...
                    <input type="number" step="0.001" min="0.001" name="exchange_rate"
                        class="h-10 w-full rounded-xl border border-gray-300 px-3 text-sm"
                ...
                <div class="md:col-span-3">
                    ...
                    <textarea name="comment" rows="2" class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                        placeholder="Comentario compra">{{ old('comment', $purchase?->comment ?? '') }}</textarea>
                </div>
                <div class="md:col-span-3">
        ...
        <script>
            function purchaseForm({
                ...
                return {
                    ...
                    get summary() {
                        const lineTotal = this.items.reduce((sum, i) => sum + ((Number(i.quantity) || 0) * (Number(i
                            .amount) || 0)), 0);
                    ...
                    setProductMeta(idx) {
                        ...
                        if (!product) return;
                        if (!this.items[idx].product_query) this.items[idx].product_query =
                            `${product.code || 'SIN'} - ${product.name}`;
                        this.items[idx].description = product.name || '';
