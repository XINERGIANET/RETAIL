    ...
    <div>
        ...
        <x-common.component-card title="Listado de compras" desc="Gestiona las compras registradas.">
            ...
            @php
                ...
                $pageTotal = (float) $currentPageItems->sum(
                    fn($purchase) => (float) ($purchase->purchaseMovement?->total ?? 0),
                );
                $pageCreditPending = (float) $currentPageItems->sum(function ($purchase) {
                    ...
                    return $paymentType === 'CREDITO' ? (float) ($purchase->purchaseMovement?->total ?? 0) : 0;
                });
            @endphp
            ...
            <div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                <form method="GET" class="flex flex-1 flex-wrap gap-3 items-center">
                    ...
                    <div class="w-full sm:w-44 xl:w-40 flex-none">
                        <x-form.date-picker id="purchases-date-from" name="date_from" placeholder="dd/mm/aaaa"
                            :defaultDate="$dateFrom" dateFormat="Y-m-d" :altInput="true" altFormat="d/m/Y" locale="es" />
                    </div>
                    <div class="w-full sm:w-44 xl:w-40 flex-none">
                        <x-form.date-picker id="purchases-date-to" name="date_to" placeholder="dd/mm/aaaa" :defaultDate="$dateTo"
                            dateFormat="Y-m-d" :altInput="true" altFormat="d/m/Y" locale="es" />
                    </div>
                    <div class="w-full sm:w-48 xl:w-44 flex-none">
                        <x-form.select-autocomplete name="payment_type" :value="$paymentType ?? ''" :options="[
                            ['value' => '', 'label' => 'Tipo pago: Todos'],
                    ...
                    <div class="flex flex-wrap gap-2">
                        <x-ui.button size="md" variant="primary" type="submit"
                            ...
                            <i class="ri-search-line text-white color_button"></i>
                            <span class="font-medium text-white">Buscar</span>
                        </x-ui.button>
                        <x-ui.link-button size="md" variant="outline"
                            href="{{ route('admin.purchases.index', $viewId ? ['view_id' => $viewId] : []) }}"
                ...
                <div class="flex flex-wrap items-start gap-2">
                    <x-ui.link-button size="md" variant="primary" style="background-color: #12f00e; color: #111827;"
                        href="{{ route('admin.purchases.create', $viewId ? ['view_id' => $viewId] : []) }}">
                ...
                <table class="w-full">
                    ...
                    <tbody>
                        @forelse ($purchases as $purchase)
                            @php
                                $purchaseDoc =
                                    strtoupper(substr($purchase->documentType?->name ?? 'C', 0, 1)) .
                                    ($purchase->purchaseMovement?->series ?? '001') .
                                    '-' .
                                    $purchase->number;
                                $paymentType = strtoupper(
                                    (string) ($purchase->purchaseMovement?->payment_type ?? 'CONTADO'),
                                );
                                $pendingAmount =
                                    $paymentType === 'CREDITO' ? (float) ($purchase->purchaseMovement?->total ?? 0) : 0;
                            @endphp
                            <tr
                                ...
                                <td class="px-5 py-4 sm:px-6 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-[11px] text-gray-500 dark:text-gray-400 uppercase font-medium">
                                            {{ $purchase->documentType?->name ?? '-' }}</p>
                                    </div>
                                </td>
                                <td class="px-5 py-4 sm:px-6 text-gray-800 text-theme-sm text-center">S/
                                    {{ number_format((float) ($purchase->purchaseMovement?->subtotal ?? 0), 2) }}</td>
                                <td class="px-5 py-4 sm:px-6 text-gray-800 text-theme-sm text-center">S/
                                    {{ number_format((float) ($purchase->purchaseMovement?->tax ?? 0), 2) }}</td>
                                <td class="px-5 py-4 sm:px-6 text-center">
                                    <p class="font-bold text-brand-600 text-theme-sm">S/
                                        {{ number_format((float) ($purchase->purchaseMovement?->total ?? 0), 2) }}</p>
                                </td>
                                <td class="px-5 py-4 sm:px-6 text-gray-800 text-theme-sm text-center truncate max-w-[150px]"
                                    title="{{ $purchase->person_name ?: '-' }}">{{ $purchase->person_name ?: '-' }}</td>

                                <td class="px-5 py-4 sm:px-6 text-gray-800 text-theme-sm text-center">
                                    {{ $purchase->moved_at ? $purchase->moved_at->format('Y-m-d H:i') : '-' }}</td>
                                <td class="px-5 py-4 sm:px-6 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form method="POST"
                                            action="{{ route('admin.purchases.destroy', array_merge([$purchase], $viewId ? ['view_id' => $viewId] : [])) }}"
                                            class="relative group js-swal-delete" data-swal-title="Eliminar compra?"
                            ...
                            <tr x-show="openRow === {{ $purchase->id }}" x-cloak
                                    ...
                                    <div class="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6 mb-4">
                                        ...
                                        <div
                                            ...
                                            <p class="mt-0.5 truncate text-sm font-medium text-gray-800 dark:text-gray-200"
                                                title="{{ $purchase->person_name ?: '-' }}">
                                                {{ $purchase->person_name ?: '-' }}</p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50">
                                            <p
                                            ...
                                            <p class="mt-0.5 text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ $purchase->moved_at ? $purchase->moved_at->format('d/m/Y H:i') : '-' }}
                                            </p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50">
                                            <p
                                                ...
                                                Usuario</p>
                                            <p
                                                class="mt-0.5 truncate text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ $purchase->user_name ?: '-' }}</p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50">
                                            <p
                                        ...
                                        <div
                                            ...
                                            <p
                                                ...
                                                Moneda</p>
                                            <p class="mt-0.5 text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ $purchase->purchaseMovement?->currency ?? 'PEN' }}</p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50">
                                            <p
                                                ...
                                                T. cambio</p>
                                            <p class="mt-0.5 text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ number_format((float) ($purchase->purchaseMovement?->exchange_rate ?? 1), 3) }}
                                            </p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50">
                                            <p
                                                ...
                                                Condición</p>
                                            <p class="mt-0.5 text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ strtoupper((string) ($purchase->purchaseMovement?->payment_type ?? 'CONTADO')) === 'CREDITO' ? 'CREDITO' : 'CONTADO' }}
                                            </p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50">
                                            <p
                                                ...
                                                Afecta Caja</p>
                                            <p class="mt-0.5 text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ ($purchase->purchaseMovement?->affects_cash ?? 'N') === 'S' ? 'Sí' : 'No' }}
                                            </p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50">
                                            <p
                                                ...
                                                Afecta Kardex</p>
                                            <p class="mt-0.5 text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ ($purchase->purchaseMovement?->affects_kardex ?? 'N') === 'S' ? 'Sí' : 'No' }}
                                            </p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50 col-span-2 lg:col-span-3">
                                            <p
                                                class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">
                                                Comentario</p>
                                            <p class="mt-0.5 truncate text-sm font-medium text-gray-800 dark:text-gray-200"
                                                title="{{ $purchase->comment ?? '-' }}">{{ $purchase->comment ?? '-' }}
                                            </p>
                                        </div>
                                    </div>

                                    <div
                                        class="rounded-xl border border-gray-200 bg-white p-0 shadow-sm dark:border-gray-800 dark:bg-white/5 overflow-hidden">
                                        <div
                                            class="bg-gray-50 px-4 py-2 border-b border-gray-200 dark:bg-gray-900/50 dark:border-gray-800">
                                            <p class="text-sm font-bold text-gray-700 dark:text-gray-200">Detalle de compra
                                            </p>
                                        </div>
                                            ...
                                            <table class="w-full">
                                                <thead>
                                                    <tr style="background-color: #334155;">
                                                        <th
                                                            class="px-4 py-2 text-left text-xs font-semibold uppercase text-white">
                                                            Código</th>
                                                        <th
                                                            class="px-4 py-2 text-left text-xs font-semibold uppercase text-white">
                                                            Descripción</th>
                                                        <th
                                                            ...
                                                            Unidad</th>
                                                        <th
                                                            class="px-4 py-2 text-right text-xs font-semibold uppercase text-white">
                                                            Cantidad</th>
                                                        <th
                                                            class="px-4 py-2 text-right text-xs font-semibold uppercase text-white">
                                                            Costo</th>
                                                ...
                                                <tbody>
                                                    @forelse($purchase->purchaseMovement?->details ?? [] as $detail)
                                                        <tr
                                                            class="border-b border-gray-100 last:border-0 dark:border-gray-800 hover:bg-gray-50/50 transition-colors">
                                                            <td
                                                                class="px-4 py-3 text-sm text-gray-800 dark:text-gray-200 font-medium">
                                                                {{ $detail->code }}</td>
                                                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">
                                                                {{ $detail->description }}</td>
                                                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">
                                                                {{ $detail->unit?->description ?? '-' }}</td>
                                                            <td
                                                                class="px-4 py-3 text-sm text-right text-gray-800 dark:text-gray-200 font-medium">
                                                                {{ number_format((float) $detail->quantity, 2) }}</td>
                                                            <td
                                                                class="px-4 py-3 text-sm text-right text-gray-800 dark:text-gray-200">
                                                                S/ {{ number_format((float) $detail->amount, 2) }}</td>
                                                            <td
                                                                class="px-4 py-3 text-sm text-right text-brand-600 dark:text-brand-400 font-bold">
                                                                S/
                                                                {{ number_format((float) $detail->quantity * (float) $detail->amount, 2) }}
                                                            </td>
                                                        </tr>
                                                    @empty
                                                        <tr>
                                                            <td colspan="6"
                                                                class="px-4 py-8 text-center text-sm text-gray-500">Sin
                                                                detalle.</td>
                                                        </tr>
                                                    @endforelse
                                                </tbody>
                            ...
                            <tr>
                                <td colspan="10" class="px-6 py-12">
                                    <div class="flex flex-col items-center gap-3 text-center text-sm text-gray-500">
                                        ...
                                        <p class="text-base font-semibold text-gray-700 dark:text-gray-200">No hay compras
                                            registradas.</p>
            ...
            <div class="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="text-sm text-gray-700 dark:text-gray-200">
                    Total filtrado:
                    <span class="font-semibold text-emerald-700 dark:text-emerald-400">S/
                        {{ number_format((float) ($purchasesTotalAmount ?? 0), 2) }}</span>
                </div>
