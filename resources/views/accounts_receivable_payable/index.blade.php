...
@section('content')
    @php
        ...
        $defaultCashRegisterId =
            (string) ($defaultCashRegisterId ?? (optional(($cashRegisters ?? collect())->first())->id ?? ''));
    ...
    <div x-data="{
            ...
            <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                <form method="GET" class="flex flex-1 flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                    ...
                    <div class="w-full sm:w-44 sm:flex-none">
                        <x-form.select-autocomplete name="status" :value="in_array($statusFilter ?? 'ALL', ['ALL', 'all']) ? 'all' : $statusFilter ?? 'all'" :options="[
                    ...
                    <div class="flex flex-wrap gap-2">
                        <x-ui.button size="md" variant="primary" type="submit" class="h-11 px-6"
                            style="background-color: #334155; border-color: #334155;">
                            <i class="ri-search-line text-gray-100"></i>
                            <span class="font-medium text-gray-100">Buscar</span>
                        </x-ui.button>
                        <x-ui.link-button size="md" variant="outline"
                            href="{{ route($baseRoute, $viewId ? ['view_id' => $viewId] : []) }}"
                            class="h-11 px-6 border-gray-200 text-gray-600 hover:bg-gray-50 hover:text-gray-900">
                    ...
                    <tbody>
                        @forelse ($records as $record)
                            @php
                                ...
                                $documentLabel = trim(
                                    ($sourceMovement?->documentType?->name ?? '-') .
                                        ' ' .
                                        ($sourceMovement?->number ?? ''),
                                );
                                $personLabel =
                                    $sourceMovement?->person_name ?: ($cashEntryMovement?->person_name ?: '-');
                                ...
                                $settlementPayload = [
                                    ...
                                    'paid' => $paid,
                                    'pending' => $pending,
                                    'preferred_cash_register_id' => str_contains(
                                        mb_strtolower(
                                            trim((string) ($sourceMovement?->documentType?->name ?? '')),
                                            'UTF-8',
                                        ),
                                        'factura',
                                    )
                            ...
                            <tr class="border-b border-gray-100 transition hover:bg-gray-50">
                                ...
                                <td class="px-5 py-4 text-center text-sm text-gray-700">
                                    {{ optional($record->date)->format('d/m/Y H:i') ?? '-' }}</td>
                                <td class="px-5 py-4 text-center text-sm text-gray-700">
                                    {{ optional($record->due_date)->format('d/m/Y H:i') ?? '-' }}</td>
                                <td class="px-5 py-4 text-center text-sm font-bold text-slate-900">S/
                                    {{ number_format($total, 2) }}</td>
                                <td class="px-5 py-4 text-center text-sm font-medium text-emerald-700">S/
                                    {{ number_format($paid, 2) }}</td>
                                <td class="px-5 py-4 text-center text-sm font-bold text-amber-700">S/
                                    {{ number_format($pending, 2) }}</td>
                                <td class="px-5 py-4 text-center">
                                    <span
                                        class="inline-flex rounded-full px-3 py-1 text-xs font-bold {{ $statusClasses }}">
                                        {{ $record->status }}
                                    </span>
                                </td>
                                <td class="px-5 py-4 text-center">
                                    @if ($pending > 0.009)
                                        <button type="button" @click="openSettlement(@js($settlementPayload))"
                                    ...
                                    @else
                                        <span
                                            class="inline-flex rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-500">
                                            Sin saldo
                                        </span>
                                    @endif
                                </td>
        ...
        <template x-teleport="body">
            <div x-show="open" x-cloak
                ...
                <div @click="closeSettlement()" class="fixed inset-0 h-full w-full bg-gray-400/30 backdrop-blur-[32px]"
                    x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
                    x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
                    x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"></div>

                <div @click.stop class="relative flex w-full max-w-4xl flex-col overflow-hidden rounded-3xl bg-[#F4F6FA]"
                    ...
                    <div class="min-h-0 flex-1 overflow-y-auto p-6 sm:p-8">
                        <div class="mb-6 flex items-start justify-between gap-4">
                            <div>
                                <p class="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Movimiento de
                                    cuenta</p>
                                <h3 class="mt-2 text-3xl font-black text-slate-800">{{ $modalTitle }}</h3>
                                <p class="mt-2 text-sm text-slate-500">
                                    Registra un {{ strtolower($rowButtonLabel) }} parcial o total y actualiza el saldo
                                    pendiente de la cuenta.
                                </p>
                            </div>
                            <button type="button" @click="closeSettlement()"
                                class="flex h-11 w-11 items-center justify-center rounded-full bg-slate-100 text-slate-400 transition hover:bg-slate-200 hover:text-slate-700">
                        ...
                        <div class="mb-5 grid gap-3 md:grid-cols-3">
                            ...
                            <div class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 shadow-sm">
                                <p class="text-[11px] font-bold uppercase tracking-[0.16em] text-amber-700">Saldo pendiente
                                </p>
                                <p class="mt-2 text-2xl font-black text-amber-700"
                                    x-text="`${account?.currency === 'USD' ? '$' : 'S/'} ${Number(account?.pending || 0).toFixed(2)}`">
                                </p>
                                <p class="mt-1 text-sm text-amber-800/80"
                                    x-text="`Pagado: ${account?.currency === 'USD' ? '$' : 'S/'} ${Number(account?.paid || 0).toFixed(2)}`">
                                </p>
                            </div>
                        </div>
                        ...
                        @if ($settlementErrors->any())
                            <div
                                class="mb-5 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
                                <p class="font-semibold">No se pudo registrar el movimiento.</p>
                                <p class="mt-1">{{ $settlementErrors->first('general') ?: $settlementErrors->first() }}
                                </p>
                            </div>
                        @endif
                        ...
                        <form method="POST" :action="formAction" class="space-y-5">
                            ...
                            <div class="grid gap-4 lg:grid-cols-3">
                                <div>
                                    <label
                                        class="mb-1.5 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Caja</label>
                                    <select name="cash_register_id" x-model="cashRegisterId"
                                    ...
                                    @if ($settlementErrors->has('cash_register_id'))
                                        <p class="mt-1 text-xs text-rose-600">
                                            {{ $settlementErrors->first('cash_register_id') }}</p>
                                    @endif
                                </div>

                                <div>
                                    <label
                                        class="mb-1.5 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Método
                                        de pago</label>
                                    <select name="payment_method_id" x-model="paymentMethodId" @change="syncMethodKind()"
                                        class="h-12 w-full rounded-xl border border-slate-300 bg-white px-4 text-sm font-medium text-slate-700">
                                    ...
                                    @if ($settlementErrors->has('payment_method_id'))
                                        <p class="mt-1 text-xs text-rose-600">
                                            {{ $settlementErrors->first('payment_method_id') }}</p>
                                    @endif
                                </div>

                                <div>
                                    <label
                                        class="mb-1.5 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Monto</label>
                                    <input type="number" step="0.01" min="0.01"
                                        :max="account ? Number(account.pending || 0) : null" name="amount"
                                        x-model="amount"
                            ...
                            <div class="grid gap-4"
                                ...
                                <div x-show="currentKind() === 'card'" x-cloak>
                                    <label
                                        class="mb-1.5 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Tarjeta</label>
                                    <select name="card_id" x-model="cardId" :required="currentKind() === 'card'"
                                    ...
                                    @if ($settlementErrors->has('card_id'))
                                        <p class="mt-1 text-xs text-rose-600">{{ $settlementErrors->first('card_id') }}
                                        </p>
                                    @endif
                                </div>
                                ...
                                <div x-show="currentKind() === 'card'" x-cloak>
                                    <label
                                        class="mb-1.5 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Pasarela</label>
                                    <select name="payment_gateway_id" x-model="paymentGatewayId"
                                        :disabled="currentKind() !== 'card'"
                                        class="h-12 w-full rounded-xl border border-slate-300 bg-white px-4 text-sm text-slate-700">
                                    ...
                                    @if ($settlementErrors->has('payment_gateway_id'))
                                        <p class="mt-1 text-xs text-rose-600">
                                            {{ $settlementErrors->first('payment_gateway_id') }}</p>
                                    @endif
                                </div>

                                <div x-show="currentKind() === 'wallet'" x-cloak>
                                    <label
                                        class="mb-1.5 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Billetera
                                        digital</label>
                                    <select name="digital_wallet_id" x-model="digitalWalletId"
                                    ...
                                    @if ($settlementErrors->has('digital_wallet_id'))
                                        <p class="mt-1 text-xs text-rose-600">
                                            {{ $settlementErrors->first('digital_wallet_id') }}</p>
                                    @endif
                                </div>

                                <div>
                                    <label
                                        class="mb-1.5 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Referencia</label>
                                    <input type="text" name="reference" x-model="reference"
                                        class="h-12 w-full rounded-xl border border-slate-300 bg-white px-4 text-sm text-slate-700"
                                        placeholder="Voucher, operación o nota">
                                    @if ($settlementErrors->has('reference'))
                                        <p class="mt-1 text-xs text-rose-600">{{ $settlementErrors->first('reference') }}
                                        </p>
                                    @endif
                                </div>
                                ...
                                <div x-show="currentKind() !== 'card' && currentKind() !== 'wallet'" x-cloak>
                                    <label
                                        class="mb-1.5 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Detalle</label>
                                    <div
                                        class="flex h-12 items-center rounded-xl border border-dashed border-slate-300 bg-white px-4 text-sm text-slate-400">
                            ...
                            <div>
                                <label
                                    class="mb-1.5 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Comentario</label>
                                <textarea name="comment" x-model="comment" rows="3"
                                    ...
                                    placeholder="Detalle opcional del movimiento de caja"></textarea>
