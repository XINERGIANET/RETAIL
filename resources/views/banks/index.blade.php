...
@section('content')
            ...
            <form method="GET" class="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <x-ui.link-button size="md" variant="outline"
                        href="{{ $viewId ? route('admin.banks.index', ['view_id' => $viewId]) : route('admin.banks.index') }}"
                ...
                @foreach ($topOperations as $operation)
                    ...
                    @if ($isCreate)
                        <x-ui.button size="md" variant="primary" type="button" style="{{ $topStyle }}"
                            @click="$dispatch('open-create-bank-modal')">
                                        ...
                                        @foreach ($rowOperations as $operation)
                                            ...
                                            @if ($isDelete)
                                                <form action="{{ $actionUrl }}" method="POST"
                                                    data-swal-title="Eliminar banco?" class="relative group js-swal-delete"
                                                    data-swal-title="Eliminar banco?"
    ...
    <x-ui.modal x-data="{ open: false }" @open-create-bank-modal.window="open = true"
        ...
        <div class="p-6 space-y-4">
            ...
            @endif
            <form id="create-bank-form" class="space-y-4"
                action="{{ $viewId ? route('admin.banks.store') . '?view_id=' . $viewId : route('admin.banks.store') }}"
                method="POST">
                @csrf
                @if ($viewId)
                    <input type="hidden" name="view_id" value="{{ $viewId }}">
    ...
    <x-ui.modal x-data="{
        ...
        <div class="p-6 space-y-4">
            ...
            <form class="space-y-4"
                :action="`{{ url('/admin/herramientas/bancos') }}/${bankData.id}{{ $viewId ? '?view_id=' . $viewId : '' }}`"
                method="POST">
                @csrf
                @method('PUT')
                @if ($viewId)
