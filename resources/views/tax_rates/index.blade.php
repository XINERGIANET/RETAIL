...
@section('content')
    ...
    <div x-data="{}">
                ...
                <table class="w-full min-w-max">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            <th class="px-3 py-3 text-left sm:px-6 first:rounded-tl-xl sticky-left-header">
                    ...
                    <tbody>
                        @forelse ($taxRates as $taxRate)
                            @php
                                ...
                                if (!empty($viewId)) {
                                    $taxRateEditUrl .=
                                        (str_contains($taxRateEditUrl, '?') ? '&' : '?') .
                                        'view_id=' .
                                        urlencode((string) $viewId);
                                    $taxRateDestroyUrl .=
                                        (str_contains($taxRateDestroyUrl, '?') ? '&' : '?') .
                                        'view_id=' .
                                        urlencode((string) $viewId);
                                }
                            @endphp
                            <tr
                                ...
                                <td class="px-3 py-4 sm:px-6 sticky-left">
                                    <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90 truncate"
                                        title="{{ $taxRate->code }}">{{ $taxRate->code }}</p>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                    <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $taxRate->description }}
                                    </p>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                    <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $taxRate->tax_rate }} %
                                    </p>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                ...
                                <td class="px-5 py-4 sm:px-6">
                                    <p class="text-gray-500 text-theme-sm dark:text-gray-400">
                                        {{ $taxRate->status ? 'Activo' : 'Inactivo' }}</p>
                                </td>
                                <td class="px-5 py-4 sm:px-6">
                                            ...
                                            @elseif ($isEdit)
                                                <div class="relative group">
                                                    <x-ui.link-button size="icon" variant="{{ $variant }}"
                                                        href="{{ $taxRateEditUrl }}" className="rounded-xl"
                                                        style="{{ $buttonStyle }}" aria-label="{{ $operation->name }}">
                                                        <i class="{{ $operation->icon }}"></i>
                                                    </x-ui.link-button>
                                                    <span
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12">
                                    <div class="flex flex-col items-center gap-3 text-center text-sm text-gray-500">
                                        ...
                                        <p class="text-base font-semibold text-gray-700 dark:text-gray-200">No hay tasas de
                                            impuesto registradas.</p>
                                        <p class="text-gray-500">Crea la primera tasa de impuesto para comenzar.</p>
                                        <x-ui.button size="sm" variant="primary" type="button"
                                            @click="$dispatch('open-tax-rate-modal')">
        ...
        <x-ui.modal x-data="{ open: false }" @open-tax-rate-modal.window="open = true"
            @close-tax-rate-modal.window="open = false" :isOpen="false" :showCloseButton="false" class="max-w-3xl">
            <div class="p-6 sm:p-8">
                <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Registrar tasa de impuesto
                            </h3>
                ...
                <form method="POST"
                    action="{{ $viewId ? route('admin.tax_rates.store') . '?view_id=' . $viewId : route('admin.tax_rates.store') }}"
                    class="space-y-6">
                    @csrf
                    @if ($viewId)
                        <input type="hidden" name="view_id" value="{{ $viewId }}">
