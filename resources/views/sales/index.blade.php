...
@section('content')
    @php
        ...
        $bootPerson = collect($invoicePeopleOptions)->first(
            fn($item) => (string) ($item['id'] ?? '') === $bootPersonId,
        );
        $invoiceModalDraftPayload =
            $invoiceModalSaleId > 0
                ? [
                    'action' => route(
                        'admin.sales.invoice',
                        array_merge([$invoiceModalSaleId], $viewId ? ['view_id' => $viewId] : []),
                    ),
    ...
    <div x-data="{
        ...
        openInvoiceModal(payload) {
            const source = this.draftPayload && String(this.draftPayload.sale_id || '') === String(payload.sale_id || '') ? { ...payload, ...this.draftPayload } :
                payload;
        ...
        <x-common.component-card title="Listado de ventas" desc="Gestiona las ventas registradas.">
            ...
            @if ($errors->has('person_id') || $errors->has('invoice_series') || $errors->has('invoice_number'))
                <div
                    class="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-800">
                ...
                <form method="GET"
                    ...
                    <div class="w-full sm:w-44 sm:flex-none">
                        <x-form.select-autocomplete name="document_type_id" :value="$selectedDocumentTypeId ?? 'all'" :options="collect($saleDocumentTypes ?? [])
                            ...
                            ->prepend(['value' => 'all', 'label' => 'Todos los docs.'])
                            ->values()
                            ->all()"
                    ...
                    <div class="w-full sm:w-40 sm:flex-none">
                        ...
                        <x-form.select-autocomplete name="cash_register_id" :value="$selectedBoxId ?? ''" :options="collect([['value' => '', 'label' => 'Todas']])
                            ->merge(
                                collect($cashRegisters ?? [])->map(
                                    fn($reg) => ['value' => $reg->id, 'label' => $reg->number],
                                ),
                            )
                            ->values()
                            ->all()"
                            placeholder="Todas" :submit-on-change="true"
                        ...
                        @php
                            ...
                            foreach ($shiftRelations as $rel) {
                                ...
                                $started = $rel->started_at
                                    ...
                                    : '';
                                $ended = $rel->ended_at
                                    ? \Carbon\Carbon::parse($rel->ended_at)->format('Y-m-d H:i:s')
                                    : '';
                    ...
                    <tbody>
                        @forelse ($sales as $sale)
                            <tr class="group/row border-b border-gray-100 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-white/5 relative hover:z-[60]"
                                ...
                                <td
                                    class="px-5 py-3 text-center sticky-left group-hover/row:bg-gray-50 dark:group-hover/row:bg-white/5">
                                ...
                                <td class="px-5 py-4 sm:px-6 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        @if ($sale->isSalesInvoice())
                                            <span
                                                class="mt-1 inline-flex rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide {{ $sale->salesBillingStatus() === 'PENDING' ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700' }}">
                                ...
                                @php
                                    $invoicePayload = [
                                        'action' => route(
                                            'admin.sales.invoice',
                                            array_merge([$sale], $viewId ? ['view_id' => $viewId] : []),
                                        ),
                                        'sale_id' => (int) $sale->id,
                                        ...
                                        'invoice_series' =>
                                            trim((string) ($sale->salesMovement?->series ?? '001')) ?: '001',
                                        'invoice_number' => trim(
                                    ...
                                    <div class="grid grid-cols-4 gap-3 sm:grid-cols-5">
                                        ...
                                        <div
                                            ...
                                            <p class="mt-0.5 text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ $sale->moved_at ? $sale->moved_at->format('d/m/Y H:i') : '-' }}</p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50">
                                            <p
                                        ...
                                        <div
                                            ...
                                            <p
                                                ...
                                                Tipo de detalle</p>
                                            <p class="mt-0.5 text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ $sale->salesMovement?->detail_type ?? '-' }}</p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50">
                                            <p
                                                ...
                                                Moneda</p>
                                            <p class="mt-0.5 text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ $sale->salesMovement?->currency ?? 'PEN' }}</p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50">
                                            <p
                                        ...
                                        <div
                                            ...
                                            <p
                                                ...
                                                Por consumo</p>
                                            <p class="mt-0.5 text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ ($sale->salesMovement?->consumption ?? 'N') === 'Y' ? 'Sí' : 'No' }}</p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50">
                                            <p
                                                ...
                                                Tipo de pago</p>
                                            <p class="mt-0.5 text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ in_array(strtoupper((string) ($sale->salesMovement?->payment_type ?? '')), ['CREDITO', 'CREDIT', 'DEUDA'], true) ? 'CREDITO' : 'CONTADO' }}
                                            </p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50">
                                            <p
                                                ...
                                                Facturación</p>
                                            <p class="mt-0.5 text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ $sale->salesBillingStatusLabel() }}</p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50 sm:col-span-2">
                                            <p
                                                class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">
                                                Comentario</p>
                                            <p class="mt-0.5 text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ Str::limit($sale->comment ?? '-', 60) }}</p>
                                        </div>
                                        <div
                                            class="rounded-lg border border-gray-200 bg-white px-4 py-2 shadow-sm dark:border-gray-700 dark:bg-gray-900/50">
                                            <p
                                                    ...
                                                    <tbody>
                                                        @foreach ($sale->salesMovement->details as $d)
                                                            ...
                                                            <tr
                                                                ...
                                                                <td
                                                                    class="px-3 py-2 text-right tabular-nums text-gray-800 dark:text-gray-200">
                                                                    {{ number_format($qty, 2) }}</td>
                                                                <td
                                                                    class="px-3 py-2 text-center text-gray-600 dark:text-gray-400">
                                                                    {{ $d->unit?->abbreviation ?? ($d->unit?->description ?? '-') }}
                                                                </td>
                                                                <td
                                                                    class="px-3 py-2 text-right tabular-nums text-gray-800 dark:text-gray-200">
                                                                    S/ {{ number_format($unitPrice, 2) }}</td>
                                                                <td
                                                                    class="px-3 py-2 text-right tabular-nums font-medium text-gray-800 dark:text-gray-200">
                                                                    S/ {{ number_format($lineTotal, 2) }}</td>
                                                            </tr>
                                                        @endforeach
                                                    </tbody>
                                                </table>
            ...
            <div class="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="text-sm text-gray-700 dark:text-gray-200">
                    ...
                    <span class="font-semibold text-emerald-700 dark:text-emerald-400">S/
                        {{ number_format((float) ($salesTotalAmount ?? 0), 2) }}</span>
                </div>
        ...
        <template x-teleport="body">
            <div x-show="invoiceModalOpen" x-cloak
                ...
                <div @click.stop class="relative flex w-full max-w-2xl flex-col overflow-hidden rounded-3xl bg-[#F4F6FA]"
                    ...
                    <form method="POST" :action="invoiceForm.action" class="flex h-full flex-col">
                        ...
                        <div class="border-b border-slate-200 px-6 py-5 sm:px-8">
                            <div class="flex items-start justify-between gap-4">
                                <div>
                                    <p class="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">Facturacion
                                    </p>
                                    <h3 class="mt-2 text-2xl font-bold text-slate-800">Registrar factura</h3>
                                    <p class="mt-1 text-sm text-slate-500">Completa el cliente, la serie y el correlativo
                                        para marcar la venta como facturada.</p>
                                </div>
                                <button type="button" @click="closeInvoiceModal()"
                                    class="inline-flex h-11 w-11 items-center justify-center rounded-full bg-slate-100 text-slate-400 transition hover:bg-slate-200 hover:text-slate-700">
                        ...
                        <div class="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 py-6 sm:px-8">
                            ...
                            <div class="rounded-2xl border border-slate-200 bg-white px-4 py-4 shadow-sm">
                                ...
                                <p class="mt-2 text-lg font-bold text-slate-800"
                                    x-text="invoiceForm.sale_code || ('Venta #' + invoiceForm.sale_id)"></p>
                            </div>
                            ...
                            <div class="space-y-1.5">
                                ...
                                <div class="relative" @click.outside="invoiceDropdownOpen = false">
                                    ...
                                    <input x-model="invoiceSearch" @focus="invoiceDropdownOpen = true"
                                        @click="invoiceDropdownOpen = true" @input="onInvoiceSearchInput()"
                                    ...
                                    <div x-show="invoiceDropdownOpen" x-cloak
                                        ...
                                        <template x-for="person in filteredPeople()" :key="`invoice-person-${person.id}`">
                                            <button type="button" @click="selectPerson(person)"
                                                class="flex w-full items-start justify-between border-b border-slate-100 px-4 py-3 text-left hover:bg-slate-50">
                                                <span class="text-sm font-medium text-slate-800"
                                                    x-text="person.label"></span>
                                                <span class="ml-3 text-xs text-slate-500"
                                                    x-text="person.document_number || 'Sin documento'"></span>
                                            </button>
                                        </template>
                                    </div>
                                </div>
                        ...
                        <div class="flex flex-wrap items-center gap-3 border-t border-slate-200 px-6 py-5 sm:px-8">
                            ...
                            <x-ui.button size="md" variant="outline" type="button" className="px-6"
