    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4">
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-3" style="height: calc(100vh - 160px);">
            ...
            <div class="lg:col-span-2 flex flex-col gap-3 overflow-hidden">
                ...
                <div class="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-800 shrink-0">
                    <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                        <div>
                            ...
                            <select id="client-id"
                                class="w-full rounded-lg border border-gray-300 bg-gray-50 px-2.5 py-2 text-xs text-gray-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:focus:border-brand-400">
                ...
                {{-- Totales --}}
                <div
                    class="rounded-lg border-2 border-brand-200 bg-gradient-to-br from-brand-50 to-brand-100 p-3 dark:border-brand-800 dark:from-brand-900/20 dark:to-brand-800/20 shrink-0">
                ...
                <div class="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <div class="grid grid-cols-3 gap-1.5">
                        @foreach ($documentTypes as $index => $documentType)
                            ...
                            @endphp
                            <button type="button"
                                class="doc-type-btn {{ $isDefaultDoc ? 'doc-active' : '' }} w-full rounded-lg border-2 {{ $isDefaultDoc ? 'border-brand-500 bg-brand-50' : 'border-gray-300 bg-gray-50' }} p-2 text-left transition hover:bg-brand-100 dark:border-gray-600 dark:bg-gray-700 dark:hover:bg-gray-600"
                                data-doc-type="{{ strtolower($documentType->name) }}" data-doc-id="{{ $documentType->id }}">
                                <div class="flex items-center gap-2">
                                    <i
                                        class="fas fa-file-alt text-base {{ $isDefaultDoc ? 'text-brand-600 dark:text-brand-400' : 'text-gray-600 dark:text-gray-400' }}"></i>
                                    <div class="flex-1">
                                        <div class="text-sm font-semibold text-gray-900 dark:text-white">
                                            {{ $documentType->name }}</div>
                                    </div>
                                    <i
                                        class="fas fa-check-circle text-sm {{ $isDefaultDoc ? 'text-brand-600 dark:text-brand-400' : 'hidden text-brand-600 dark:text-brand-400' }}"></i>
                                </div>
                            </button>
                        @endforeach
                    </div>
            ...
            docButtons.forEach(btn => {
                btn.addEventListener('click', function() {
                    docButtons.forEach(b => {
                        ...
                        b.classList.add('border-gray-300', 'bg-gray-50');
                        b.classList.remove('border-brand-500', 'bg-brand-50');
                        b.querySelector('.fa-file-alt').classList.remove('text-brand-600',
                            'dark:text-brand-400');
                    ...
                    this.querySelector('.fa-file-alt').classList.remove('text-gray-600',
                        'dark:text-gray-400');
                    this.querySelector('.fa-file-alt').classList.add('text-brand-600',
                        'dark:text-brand-400');
